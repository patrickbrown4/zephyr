...
for case in runcases:
    ...
    # load_scaler = cases.loc[case,'load_scaler']
    load_scaler = 1

    ###### Distance parameters
    distancepath = os.path.join(
        projpath,'io','cf-2007_2013','{}x',''
    ).format(interconnection_scaler) + os.sep

    ### PV assumptions
    pvpath = os.path.join(
        projpath,'io','cf-2007_2013','{}x','{}','pv','{}-{}t-{:.0f}az','binned',
    ).format(interconnection_scaler, unitlevel, systemtype, axis_tilt, axis_azimuth) + os.sep
    ...
    ).format(interconnection_scaler, unitlevel, modelsave) + os.sep
