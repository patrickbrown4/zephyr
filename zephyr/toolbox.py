...
timezone_state = {
    'ME': -5, 'NH': -5, 'VT': -5, 'MA': -5, 'NY': -5,
    'CT': -5, 'RI': -5, 'PA': -5, 'NJ': -5, 'DE': -5, 
    'MD': -5, 'MI': -5, 'IN': -5, 'OH': -5, 'WV': -5, 
    'VA': -5, 'NC': -5, 'SC': -5, 'GA': -5, 'FL': -5,
    'KY': -5, 
    'TN': -6, 'ND': -6, 'SD': -6, 'MN': -6, 'WI': -6, 
    'NE': -6, 'IA': -6, 'IL': -6, 'KS': -6, 'MO': -6,
    'OK': -6, 'AR': -6, 'TX': -6, 'LA': -6, 'MS': -6,
    'AL': -6, 
    'MT': -7, 'ID': -7, 'WY': -7, 'UT': -7, 'CO': -7, 
    'AZ': -7, 'NM': -7,
    'WA': -8, 'OR': -8, 'CA': -8, 'NV': -8, 
    'AK': -9, 'HI': -10, 'DC': -5,
    }
...
tz_state = {key: timezone_to_tz(timezone_state[key]) for key in timezone_state}
...
### Dictionary to get hour from day index
hour2day = dict(zip(range(8784),[i//24 for i in range(8784)]))
...
def convert_latlonpoly(polyin, projection=None, proj='aea'):
    """
    Notes
    -----
    * polyin must be a shapely Polygon or MultiPolygon
    * results are returned in km^2
    """
    import shapely, pyproj
    ### Make a projection if necessary
    if projection is None:
        raise Exception('not implemented yet')
    ### Easy way, if simple polygon
    if type(polyin) == shapely.geometry.polygon.Polygon:
        ### Get coords, project into output coordinates
        coord = polyin.exterior.coords
        lon,lat = zip(*coord)
        x,y = projection(lon,lat)
        ### Make new polygon in output coordinates
        cop = {'type':'Polygon','coordinates':[zip(x,y)]}
        ### Calculate the area [km^2]
        polyout = shapely.geometry.shape(cop)
    ### If multipolygon
    elif type(polyin) == shapely.geometry.multipolygon.MultiPolygon:
        subpolys = []
        for subpoly in polyin:
            ### Get coords, project into output coordinates
            ...
            ### Make new polygon in output coordinates
            cop = {'type':'Polygon','coordinates':[zip(x,y)]}
            subpolys.append(shapely.geometry.shape(cop))
        ### Create the output multipoly
        polyout = shapely.geometry.MultiPolygon(subpolys)
    return polyout
...
def polyoverlaparea_km2(poly1, poly2=None, method='intersection', returnpoly=False):
    ...
    if polyout.is_empty:
        area = 0
    elif type(polyout) in [shapely.geometry.point.Point,
    ...
    ### If multipolygon
        ...
        for subpoly in polyout:
            ...
            if type(subpoly) in [shapely.geometry.point.Point,
                                 shapely.geometry.linestring.LineString]:
                continue
            ### Get coords, bounds, and centroid
            ######## 20200128
    ...
    ### Catch errors
        ...
        print(type(polyout))
        print('len(polyout) = {}'.format(len(polyout)))
        raise Exception('Unsupported poly type: {}'.format(type(polyout)))
