...
#%%### Functions
#%% Full PCM
def pcm(system, solver=None, hours=None, verbose=2, includedual=False, **solverkwargs):
    """
    ...
    model = pulp.LpProblem("Linear dispatch", pulp.LpMinimize)
    ...
    for period in periods:
        ...
        hour2day = dict(
            zip(range(hours[period]), [i // 24 for i in range(hours[period])])
        )
        ##### Generators
        for gen in gens:
            #### Availability
            ...
            if gens[gen].perfectly_rampable:
                ...
                for hour in range(1, hours[period] + 1):
                    model += (
                        genpower[gen, period, hour] - genpower[gen, period, hour - 1]
            ...
            for hour in range(hours[period]):
                if gens[gen].always_available:
                    model += (
                        genpower[gen, period, hour] >= gens[gen].cap * gens[gen].gen_min
                    )
                else:
                    model += genpower[gen, period, hour] >= (
                        gens[gen].cap
                        * gens[gen].availability[period][hour]
                        * gens[gen].gen_min
                    )
        ...
        for stor in stors:
            for hour in range(hours[period]):
                ...
                if system.cycling_reserves == False:
                    model += storcharge[stor, period, hour] + stordischarge[
                        stor, period, hour
                    ] <= (stors[stor].cappower)
                else:
                    model += (
                        storcharge[stor, period, hour]
                        + stordischarge[stor, period, hour]
                ...
                for hour in range(1, hours[period] + 1):
                    model += (
                        (
                            ...
                            - stordischarge[stor, period, hour - 1]
                            * (1 / stors[stor].efficiency_discharge)
                            + storcharge[stor, period, hour - 1]
                            * stors[stor].efficiency_charge
                        )
            ...
            ### Year-end wrap
            model += (
                storenergy[stor, period, 0] == storenergy[stor, period, hours[period]]
            )
            model += (
                storcharge[stor, period, 0] == storcharge[stor, period, hours[period]]
            )
            model += (
                stordischarge[stor, period, 0]
                == stordischarge[stor, period, hours[period]]
            )
        ...
        for node in loads[period]:  ### Load is labeled by node name
            for hour in range(hours[period]):
                model += (
                    ...
                    + pulp.lpSum(
                        [
                            flowneg[line, period, hour] * (1 - lines[line].loss)
                            for line in lines
                            if lines[line].node1 == node
                        ]
                    )
                    ...
                    + pulp.lpSum(
                        [
                            flowpos[line, period, hour] * (1 - lines[line].loss)
                            for line in lines
                    ...
                    - pulp.lpSum(
                        [
                            flowneg[line, period, hour]
                            for line in lines
                            if lines[line].node2 == node
                        ]
                    )
                ) == loads[period][node][hour], "price_p{}_l{}_h{}".format(
                    period, node, hour
                )
        ...
        for line in lines:
            for hour in range(hours[period]):
                ...
                model += (flowpos[line, period, hour] + flowneg[line, period, hour]) * (
                    1 - lines[line].loss
                ) <= lines[line].capacity
        ...
        for res in reshydros:
            ...
            for hour in range(hours[period]):
                ...
                model += (
                    ...
                    >= min(
                        reshydros[res].capacity_bound_hi * reshydros[res].gen_min,
            ...
            if reshydros[res].perfectly_rampable:
                ...
                for hour in range(1, hours[period] + 1):
                    model += (
                        respower[res, period, hour] - respower[res, period, hour - 1]
    ...
    if system.has_co2cap:
        model += (
            pulp.lpSum(
                [
                    genpower[gen, period, hour] * gens[gen].emissionsrate
                    for gen in gens
                    for period in periods
                    for hour in range(hours[period])
                ]
    ...
    output_operation = {
        period: {
            ...
            **{
                stor
                + "_charge": [
                    -storcharge[stor, period, hour].varValue
                    for hour in range(hours[period] + 1)
                ]
                for stor in stors
            },
            **{
                stor
                + "_discharge": [
                    stordischarge[stor, period, hour].varValue
                    for hour in range(hours[period] + 1)
                ]
                for stor in stors
            },
            **{
                stor
                + "_energy": [
                    storenergy[stor, period, hour].varValue
                    for hour in range(hours[period] + 1)
                ]
                for stor in stors
            },
            **{
                res
                + "_power": [
                    respower[res, period, hour].varValue
                    for hour in range(hours[period] + 1)
                ]
                for res in reshydros
            },
            **{
                res
                + "_spill": [
                    spillpower[res, period, hour].varValue
                    for hour in range(hours[period] + 1)
                ]
                for res in reshydros
            },
            **{
                "Load_"
                + load: [loads[period][load][hour] for hour in range(hours[period])]
                + [np.nan]
                for load in loads[period]
            },
            **{
                line
                + "_pos": [
                    flowpos[line, period, hour].varValue
                    for hour in range(hours[period] + 1)
                ]
                for line in lines
            },
            **{
                line
                + "_neg": [
                    -flowneg[line, period, hour].varValue
                    for hour in range(hours[period] + 1)
                ]
                for line in lines
            },
    ...
    output_values = {
        ...
        "co2shadowprice": (
            -model.constraints["co2cap"].__dict__["pi"] * 1e6
            if system.has_co2cap
            else 0
        ),
        ### LCOE in $/MWh
        ...
        "co2rate": sum(
            [
                (
                    np.array(output_operation[period][gen]) * gens[gen].emissionsrate
                ).sum()
                for period in periods
                for gen in gens
            ]
        ...
        "loadpeak": max(
            [
                sum([loads[period][load] for load in loads[period]]).max()
                for period in periods
            ]
        ),
    ...
    if includedual:
        dfdual = pd.DataFrame(
            ...
            data={
                "pi": [
                    model.modifiedConstraints[i].pi
                    for i in range(len(model.modifiedConstraints))
                ],
                "slack": [
                    model.modifiedConstraints[i].slack
                    for i in range(len(model.modifiedConstraints))
                ],
            },
        )
