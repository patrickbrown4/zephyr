...
def download_lmps(year, iso, market, overwrite=False, sleeptime=60,
    ...
    * ERCOT LMPs more than 30 days old must be requested from ERCOT. 
    ...
    projpath + 'ERCOT/in/lmp/{}/{}/'.format(market, year)
    ...
    ### Set file structure
    if filepath is None:
        filepath = projpath+'{}/in/lmp/{}/'.format(iso, market)
    ...
    files = {
        'ISONE': zephyr.toolbox.makedays(year),
        'MISO': zephyr.toolbox.makedays(year),
        'PJM': zephyr.toolbox.makedays(year),
...
def nodalize(year, market, iso,
    ...
    if iso.upper() == 'CAISO':
        ...
        if filepathout == None:
            filepathout = projpath+'{}/io/{}-nodal/{}/'.format(
                iso, product.lower(), market)

            if (market == 'rt') and (submarket == 'RTM'):
                filepathout = projpath+'{}/io/{}-nodal/{}-month/'.format(
                    iso, product.lower(), market)
        ...
        if (filepathin == None) and (market == 'da'):
            filepathin = projpath+'{}/in/lmp/{}/{}/'.format(iso, market, year)

        elif (filepathout == None) and (market == 'rt'):
            filepathout = projpath+'{}/io/lmp-nodal/{}-month/'.format(iso, market)
        elif filepathout == None:
            filepathout = projpath+'{}/io/lmp-nodal/{}/'.format(iso, market)

    else:
        if filepathin == None:
            filepathin = projpath+'{}/in/lmp/{}/'.format(iso, market)
        if filepathout == None:
            filepathout = projpath+'{}/io/lmp-nodal/{}/'.format(iso, market)
    ...
    if not os.path.exists(projpath+'{}/io/missingnodes/'.format(iso.upper())):
        os.makedirs(projpath+'{}/io/missingnodes/'.format(iso.upper()), exist_ok=True)
    if not os.path.exists(projpath+'{}/io/datatimes/'.format(iso.upper())):
        os.makedirs(projpath+'{}/io/datatimes/'.format(iso.upper()), exist_ok=True)
    if not os.path.exists(projpath+'{}/io/fulltimenodes/year/'.format(iso.upper())):
        os.makedirs(projpath+'{}/io/datatimes/'.format(iso.upper()), exist_ok=True)
    if not os.path.exists(projpath+'{}/io/fulltimenodes/day/{}/'.format(iso.upper(), market)):
        os.makedirs(projpath+'{}/io/fulltimenodes/day/{}/'.format(iso.upper(), market), 
            exist_ok=True)
    print(filepathout)
    ...
    ### Shared components
    nodesfiles = {
        'CAISO': projpath+'CAISO/io/caiso-node-latlon.csv',
        'ERCOT': projpath+'ERCOT/io/ercot-node-latlon.csv',
        #'MISO':  projpath+'MISO/in/miso-node-map.csv',
        'MISO':  projpath+'MISO/io/miso-node-latlon.csv',
        # 'PJM':   projpath+'PJM/io/pjm-pnode-latlon-uniquepoints.csv',
        'PJM':   projpath+'PJM/io/pjm-node-latlon.csv',
        'NYISO': projpath+'NYISO/io/nyiso-node-latlon.csv',
        'ISONE': projpath+'ISONE/io/isone-node-latlon.csv' 
    }
    ...
    # for file in files[-3:]:
    #     print(file)
    timezone = zephyr.toolbox.tz_iso[iso]
    res = resolution[iso][market]
    
    ### Make the inputs easier to work with
    iso = iso.upper()
    hours = zephyr.toolbox.yearhours(year)
    dates = zephyr.toolbox.makedays(year)
    ...
    if iso == 'ISONE':
        ...
        newtime = list(pd.date_range(dates[0], freq='H', periods=zephyr.toolbox.yearhours(year)))
        ...
        fullindex = pd.date_range(dates[0], freq='H', periods=zephyr.toolbox.yearhours(year))
        ...
        for i in trange(len(files)):
            colnames = ['PnodeID']
            if dates[i] not in [zephyr.toolbox.dst_springforward[year], zephyr.toolbox.dst_fallback[year]]:
                for j in range(24):
                    colnames.append(dates[i] + 'H{:02d}'.format(j))
                dfin = pd.read_csv(
            ...
            elif dates[i] == zephyr.toolbox.dst_springforward[year]:
                for j in range(23):
                    colnames.append(dates[i] + 'H{:02d}'.format(j))
                dfin = pd.read_csv(
                    files[i], skiprows=skiprows[market], header=None, 
            ...
            elif dates[i] == zephyr.toolbox.dst_fallback[year]:
                for j in range(25):
                    colnames.append(dates[i] + 'H{:02d}'.format(j))
                dfin = pd.read_csv(
                    files[i], skiprows=skiprows[market], header=None, 
        ...
        if market == 'da':
            ...
            def dstfallback(dataframe):
                fallback = zephyr.toolbox.dst_fallback[year]
            ...
            def makeindexconvert(files, dates):
                ...
                for i in trange(len(files)):
                    ...
                    if dates[i] == zephyr.toolbox.dst_fallback[year]:
                        # print(df.head())
                        df = dstfallback(df)
            ...
            datesprev = zephyr.toolbox.makedays(year - 1)
            datesthis = zephyr.toolbox.makedays(year)
            ...
            dfall.index = dfall.index.tz_localize(zephyr.toolbox.tz_iso[iso])

            ### Fix DST
            dststart = dfall.index.get_loc(zephyr.toolbox.dst_springforward[year] + ' 01:55')
            ...
            if year == 2012:
                dstend = dfall.index.get_loc(zephyr.toolbox.dst_fallback[year] + ' 01:59:34')
            else:
                dstend = dfall.index.get_loc(zephyr.toolbox.dst_fallback[year] + ' 01:55')
        ...
        if market == 'da':
            ...
            datesin = zephyr.toolbox.makedays(year)
            ...
            dfall.index = (
                dfall.index
                .tz_localize('UTC')
                .tz_convert(zephyr.toolbox.tz_iso[iso]))
            ...
            for month in months:
                datetimesin = pd.date_range(
                    start='{}{:02}01T{:02}:00'.format(year, month, abs(zephyr.toolbox.timezone_iso[iso])),
                    periods = zephyr.toolbox.monthhours(year, month),
                    freq = 'H')
                ...
                niceindex = pd.date_range(
                    ...
                    periods = (zephyr.toolbox.monthhours(year, month) * 60 / res),
                    freq = '5T',
                    tz=zephyr.toolbox.tz_iso[iso])
                ...
                dates = pd.date_range(
                    ...
                    periods = (zephyr.toolbox.monthhours(year, month) / 24),
                    freq = '1D')
            ...
            ### Set new filepaths
            filepathin = projpath+'{}/io/lmp-nodal/{}-month/'.format(iso, market)
            filepathout = projpath+'{}/io/lmp-nodal/{}/'.format(iso, market)
        ...
        def datetimefromfile(file, clean=False):
            ...
            springforward_date = zephyr.toolbox.dst_springforward[year]
            fallback_date = zephyr.toolbox.dst_fallback[year]
        ...
        if market == 'da':
            ...
            # if len(dfall.index.unique()) != zephyr.toolbox.yearhours(year):
            #   raise Exception("len(dfall.index.unique() != zephyr.toolbox.yearhours(year)")
            ...
            filekey = projpath+'{}/io/{}-{}-filekey-cleaned-existing.csv'.format(
            ...
            if (os.path.exists(filekey)) and (len(pd.read_csv(filekey, usecols=[2])) >= 743827):
                ...
                dffiles.to_csv(
                    projpath+'{}/io/{}-{}-filekey-cleaned-existing.csv'.format(
                        iso.upper(), iso.lower(), market),
                    index=False)
            ...
            for month in months:
                ...
                yearindex = pd.date_range(
                    start='{}-01-01 00:00'.format(year),
                    periods = int(zephyr.toolbox.yearhours(year) * 60 / res),
                    freq = '{}T'.format(res),
                #     tz=zephyr.toolbox.tz_iso[iso]
                )
                ...
                dfmonth = dffiles.loc[
                    ...
                    & (dffiles['Datetime-Binned']
                        <= (
                            pd.Timestamp('{}-{:02}-01 00:00'.format(year, month))) 
                            + pd.Timedelta('{}H'.format(zephyr.toolbox.monthhours(year, month) + 1)))
                ]
            ...
            for node in tqdm(uniquenodes):
                ...
                dfyear.index = dfyear.index.tz_localize(zephyr.toolbox.tz_iso[iso])
    ...
    if (iso in ['CAISO', 'ERCOT'] and (market == 'rt')):
        ...
        pd.Series(missingnodes).to_csv(
            projpath+'{}/io/missingnodes/{}-{}lmp-{}-{}{:02}.csv'.format(
        ...
        pd.DataFrame(datalength).to_csv(
            projpath+'{}/io/datatimes/{}-{}lmp-{}-{}{:02}.csv'.format(
                iso.upper(), iso.lower(), market, submarket, year, month), 
        ...
        fulltime = fulltime[
            fulltime[1] == zephyr.toolbox.monthhours(year, month) * 12]
        fulltime.to_csv(
            ### Original
            # projpath+'{}/io/fulltimenodes/year/{}-{}lmp{}-{}{:02}.csv'.format(
            ### New
            projpath+'{}/io/fulltimenodes/{}-{}lmp{}-{}{:02}.csv'.format(
                iso.upper(), iso.lower(), market, 
        ...
        ## List of nodes from nodemap that don't have LMP data
        pd.Series(missingnodes).to_csv(
            projpath+'{}/io/missingnodes/{}-{}lmp-missing-{}.csv'.format(
                iso.upper(), iso.lower(), market, year), 
            index=False)

        ## Intervals (hours or 5-min chunks) of LMP data per node
        pd.DataFrame(datalength).to_csv(
            projpath+'{}/io/datatimes/{}-{}lmp-datatimes-{}.csv'.format(
                iso.upper(), iso.lower(), market, year), 
            index=False, header=False)
        ...
        fulltime.to_csv(
            ### Original
            # projpath+'{}/io/fulltimenodes/year/{}-{}lmp-fulltime-{}.csv'.format(
            ### New
            projpath+'{}/io/fulltimenodes/{}-{}lmp-fulltime-{}.csv'.format(
                iso.upper(), iso.lower(), market, year), 
            index=False, header=False, columns=[0])
    ...
    if iso == 'ISONE':
        for i in trange(len(dates)):
            ...
            pd.Series(fulltimenodes).to_csv(
                projpath+'{}/io/fulltimenodes/day/{}/{}.csv'.format(
        ...
        for date in fulldaynodes:
            ...
            pd.Series(nodes).to_csv(
                projpath+'{}/io/fulltimenodes/day/{}/{}.csv'.format(iso.upper(), market, date),
                index=False)
...
def nodelocations_pjm():
    ...
    dfin.to_csv(projpath+'PJM/in/zip-code-mapping.csv', index=False)

    ## Save the unique zip codes
    df = pd.read_csv(projpath+'PJM/in/zip-code-mapping.csv', dtype={'Zip Code': 'category'})
    df['Zip Code'] = df['Zip Code'].map(lambda x: '{:>05}'.format(x))
    zips_unique = pd.Series(df['Zip Code'].unique())
    zips_unique.to_csv(projpath+'PJM/io/zips-pjm-unique.csv', index=False)
    ...
    zipcodes = pd.read_csv(
        projpath+'PJM/io/zips-pjm-unique.csv', dtype='category', 
    ...
    zipgeo.to_csv(projpath+'PJM/io/zips-latlon-pjm.csv', index=False)

    ###### Determine node locations
    ### Load input files
    zips = pd.read_csv(
        projpath+'PJM/io/zips-latlon-pjm.csv', 
        dtype={'zipcode': 'category'}, index_col='zipcode')

    dfnodes = pd.read_csv(projpath+'PJM/in/zip-code-mapping.csv', dtype='category')
    ...
    dfout.to_csv(projpath+'PJM/io/pjm-node-latlon.csv', index=False)
    latlons[['latlonindex', 'latitude', 'longitude']].to_csv(
        projpath+'PJM/io/pjm-pnode-unique-latlons-for-nsrdb.csv', index=False)
...
def nodelocations_caiso():
    ...
    nodemapxml = projpath+'CAISO/in/GetPriceContourMap.xml'
    if not os.path.exists(nodemapxml):
        print("Need to download the input file by hand from "
            ...
            " and save it at (projpath + 'CAISO/in/GetPriceContourMap.xml).")
        ...
        # xmlfile = projpath+'CAISO/in/GetPriceContourMap.xml'
    ...
    ### Write output
    dfclean.to_csv(projpath+'CAISO/io/caiso-node-latlon.csv', index=False)
    return dfclean

def nodelocations_miso(save=True):
    import geopandas as gpd
    ### Test if nodemap json files exist and download if not
    filepaths = {
        'Nodes': projpath + 'MISO/in/MISO_GEN_INT_LZN.json',
        'Hubs':  projpath + 'MISO/in/PNODELMPLabels_2.json',
        'ReserveZones': projpath + 'MISO/in/ASMZones_2.json',
        'PlanningZones': projpath + 'MISO/in/Planning_Zones.json',
    }
    ...
    if save:
        dfout.to_csv(projpath + 'MISO/io/miso-node-latlon.csv', index=False)
    return dfout
...
def nodelocations_isone(filepath_input=None):
    ...
    if (filepath_input==None) or (os.path.exists(filepath_input)==False):
        print("Example: projpath + 'ISONE/in/nepnode_lat_long.xlsx'. "
    ...
    dfout.to_csv(projpath+'ISONE/io/isone-node-latlon.csv', index=False)
    return dfout
...
def nodelocations_ercot(filepath_input=None):
    ...
    if (filepath_input==None) or (os.path.exists(filepath_input)==False):
        print("Missing input file. To get this file, register as an IMRE at "
            ...
            "Example: projpath+'ERCOT/in/rpt.00011205.0000000000000000.20170530"
    ...
    ### Write outputs
    dfout.to_csv(projpath+'ERCOT/io/ercot-node-latlon.csv', index=False)
    latlons[['latlonindex', 'latitude', 'longitude']].to_csv(
        projpath+'ERCOT/io/ercot-node-latlon-unique.csv', index=False)
    return dfout
...
def nodelocations_nyiso():
    ...
    ### Download files
    for year in years:
        filepathout = projpath + 'NYISO/in/' + os.path.dirname(urls[year]).split('/')[-1]
        urllib.request.urlretrieve(urls[year], filepathout)

    ### Concat input files into clean csv
    filesin = {year: projpath + 'NYISO/in/' + os.path.dirname(urls[year]).split('/')[-1]
               for year in years}
    ...
    # codes.to_csv(projpath + 'NYISO/test/io/nyiso-county-codes.csv')
    ...
    ### Write outputs
    geo.to_csv(projpath+'NYISO/io/nyiso-locations-latlon.csv', index=False)
    dfout.to_csv(projpath+'NYISO/io/nyiso-node-latlon.csv', index=False)
    latlons[['latlonindex', 'latitude', 'longitude']].to_csv(
        projpath+'NYISO/io/nyiso-node-unique-latlons-for-nsrdb.csv', index=False)
    return dfout
...
def makefolders():
    ...
    for iso in isos:
        os.makedirs(projpath+'{}/in/'.format(iso), exist_ok=True)
        os.makedirs(projpath+'{}/io/'.format(iso), exist_ok=True)
    os.makedirs(projpath+'io/', exist_ok=True)
    os.makedirs(projpath+'out/', exist_ok=True)
    os.makedirs(projpath+'figs/', exist_ok=True)
    os.makedirs(projpath+'Validation/out/', exist_ok=True)
...
def copyfiles():
    ...
    * results files are in USD2017. ALREADY CORRECTED FOR INLATION.
    ...
    os.makedirs(datapath+'EPA/so2/', exist_ok=True)
    ...
    shutil.copy(dirmodule+'../data/RGGI/AllowancePricesAndVolumes.csv',
                datapath+'RGGI/')
    ### EPA SO2 price
    shutil.copy(dirmodule+'../data/EPA/so2/so2_auction_prices.xlsx',
                datapath+'EPA/so2/')
    ### Core results
    os.makedirs(projpath+'out/', exist_ok=True)
    files = glob(dirmodule+'../results/*')
    for file in files:
        shutil.copy(file, projpath+'out/')
    ### ISO Load
    ...
    for market in markets:
        for iso in isos:
            ...
            if year is not None:
                years = [year]
            for i_year in years:
                df = zephyr.io.getdflmp(iso, market, i_year).dropna(axis=1)
                ...
                for column in tqdm(
                    iterable, leave=True, desc='{} {} {} {}'.format(
                        iso, market, i_year, product)):
                    filepath = projpath+'{}/io/lmp-nodal/{}/'.format(
                        iso.upper(), market)
                ...
                fulltimeoutpath = projpath+'{}/io/fulltimenodes/'.format(iso)
                ...
                meanoutpath = projpath+iso+'/io/lmp-nodal-mean/'
